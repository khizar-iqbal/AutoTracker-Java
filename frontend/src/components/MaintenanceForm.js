...
const MaintenanceForm = ({ fetchRecords }) => {
    ...
    const navigate = useNavigate(); // To navigate back to the ViewRecordsPage
    ...
    const submitHandler = async (e) => {
        ...
        try {
            const newRecord = { carDetails, workDescription, paymentStatus };
            await axios.post('http://localhost:5000/api/maintenance', newRecord);
            fetchRecords();
            setSuccess('Record added successfully');
            setCarDetails({ number: '', color: '', model: '' });
            setWorkDescription('');
            setPaymentStatus({ received: false, paymentMethod: 'Cash' });
        } catch (error) {
    ...
    return (
        <div className="container mt-5">
            <div className="card shadow-lg p-4">
                <h2 className="text-center text-primary mb-4">Add Maintenance Record</h2>
                <form onSubmit={submitHandler}>
                    ...
                    <button type="submit" className="btn btn-primary w-100 btn-lg">Add Record</button>
                </form>
